# ARM-A AUTORESEARCH III — G4 CONFIRMATION CELL
#
...
#
# One question: DOES THE CANDIDATE BEAT THE CERTIFIED PACKED b1024 BASELINE
...
#
# Candidate: identical math with three exact execution changes:
...
#   3. (CANDIDATE_FLAGS below) paper_y direct layout + cached RoPE phase,
#      enabled only if local evidence promotes them.
#
...
def main():
    gate = tiny_packed_correctness()
    require(gate["status"] == "PASS", "tiny packed correctness gate failed")
    print("TINY_GATE=" + json.dumps(gate, sort_keys=True, separators=(",", ":")))

    model_gate = tiny_model_gate()
    require(model_gate["status"] == "PASS", "tiny packed model gate failed")
    print(
        "TINY_MODEL_GATE="
        + json.dumps(model_gate, sort_keys=True, separators=(",", ":"))
    )
